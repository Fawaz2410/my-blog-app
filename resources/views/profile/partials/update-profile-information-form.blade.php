<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600">
            {{ __("Update nama, foto profil, dan alamat email akun Anda.") }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div class="flex items-center gap-4">
            <div class="shrink-0">
                @if(Auth::user()->avatar)
                    <img class="h-16 w-16 object-cover rounded-full border border-gray-200" src="{{ asset('storage/' . Auth::user()->avatar) }}" alt="Foto Profil" />
                @else
